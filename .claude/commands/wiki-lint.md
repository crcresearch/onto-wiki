...
8. **Finish the cycle.** Commit in the wiki's own git repo in two steps, without asking. One commit per log entry keeps `git blame` on the log a faithful per-entry record (see "Log Entry Attribution" in SCHEMA):
    ```
    git -C wiki/onto-wiki.wiki add <lint-fix-and-index-files-by-name>
    git -C wiki/onto-wiki.wiki commit -m "lint: <summary>"
    git -C wiki/onto-wiki.wiki add log_onto-wiki.md
    git -C wiki/onto-wiki.wiki commit -m "log: lint <summary>"
    ```
